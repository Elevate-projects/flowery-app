...
import 'package:flowery_app/data/repositories/cart/cart_repositories_impl.dart';
...
void main() {
 ...
 late final RemoteDataSourceGetLoggedUser remoteDataSourceGetLoggedUser;
  ...
  test('when call getLoggedUserCart it should be called successfully from RemoteDataSourceGetLoggedUser and return GetLoggedUserCartEntity', ()async {
        ...
        cartItems: [
          CartItemEntity(
            product: ProductEntity(
              ...
              quantity: 50,
              category: "673c46fd1159920185",
              occasion: "673c46fd1159920185",
              v: 1,
              sold: 20,
            ),
            price: 500,
    ...
    when(
      remoteDataSourceGetLoggedUser.getLoggedUserCart("token"),
    ).thenAnswer((_) async => expectedCartResult);
    final result = await cartRepositoryImpl.getLoggedUserCart("token");
    expect(result, isA<Success<GetLoggedUserCartEntity>>());
    expect(result, expectedCartResult);
    verify(remoteDataSourceGetLoggedUser.getLoggedUserCart("token")).called(1);
  });
}